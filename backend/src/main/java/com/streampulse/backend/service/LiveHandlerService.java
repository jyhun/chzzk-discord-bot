...
public class LiveHandlerService {
    ...
    public void handleStart(Set<String> startIds, Map<String, LiveResponseDTO> dtoMap, AtomicBoolean firstRun) {
        ...
        try {
            ...
            if (!newStreamers.isEmpty()) {
                ...
                log.info("[handleStart] startIds 개수: {}", startIds.size());
                log.info("[handleStart] 새로 조회된 streamerMap 사이즈: {}", streamerMap.size());
            }
            ...
            if (!newSessions.isEmpty()) {
                ...
                List<StreamSession> savedSessions = streamSessionService.findAllByStreamerIds(startIds);
                log.info("다시 불러온 세션 수: {}", savedSessions.size());
            }
        } catch (Exception e) {
        ...
        log.info("[handleStart] 실행 시간: {} ms", elapsed);
    ...
    public void handleTopic(Set<String> nextIds, Map<String, LiveResponseDTO> dtoMap, AtomicBoolean firstRun) {
        ...
        long elapsed = System.currentTimeMillis() - startTime;
        log.info("[handleTopic] 실행 시간: {} ms", elapsed);
    ...
    public void handleEnd(Set<String> endIds) {
        ...
        long elapsed = System.currentTimeMillis() - startTime;
        log.info("[handleEnd] 실행 시간: {} ms", elapsed);
