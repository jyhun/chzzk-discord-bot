...
public interface SubscriptionRepository extends JpaRepository<Subscription, Long> {
    ...
    @Query("SELECT s FROM Subscription s " +
            "WHERE s.streamer IS NULL AND s.eventType = :eventType AND s.active = true")
    List<Subscription> findGlobalSubscriptionsByEventType(@Param("eventType") EventType eventType);

    @Query("SELECT s FROM Subscription s " +
            "WHERE s.streamer.channelId = :streamerId AND s.eventType = :eventType AND s.active = true")
    List<Subscription> findStreamerSubscriptionsByEventType(
    ...
    @Query("SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END " +
            ...
            "AND s.streamer IS NULL AND s.eventType = :eventType AND s.active = true")
    ...
    @Query("SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END " +
            ...
            "AND s.eventType = :eventType AND s.active = true")
    ...
    @Query("SELECT s FROM Subscription s " +
            "WHERE s.discordChannel.discordChannelId = :discordChannelId " +
            "AND s.active = true")
    List<Subscription> findActiveSubscriptionsByChannelId(
            @Param("discordChannelId") String discordChannelId
    );
    ...
    @Query("SELECT s FROM Subscription s " +
            ...
            "AND s.eventType = :eventType AND s.active = true")
    List<Subscription> findActiveSubscriptionsByChannelIdAndEventType(
            @Param("discordChannelId") String discordChannelId,
            @Param("eventType") EventType eventType
    );
    ...
    @Query("SELECT s FROM Subscription s " +
            ...
            "AND s.eventType = :eventType AND s.active = true")
    List<Subscription> findActiveSubscriptionsByChannelIdAndStreamerIdAndEventType(
            @Param("discordChannelId") String discordChannelId,
            @Param("streamerId") String streamerId,
            @Param("eventType") EventType eventType
    );
